...
- **Safe ownership**: only files recorded in `mars.lock` are ever touched by mars. Orphan cleanup is driven off the _previous_ lock, not a filesystem scan — mars will never delete something it didn't originally write. This is an explicit invariant in `AGENTS.md` / `CLAUDE.md`.
...
| 6   | `sync_targets`  | Copies `.mars/` → each configured target (`.agents/`, `.claude/`, …). Non‑fatal per‑target: a failure on one target doesn't block others. Orphan cleanup uses _previous_ lock to know what's safe to remove. |
...
3. **Dry‑run mode first.** Add `bttrai install --dry-run --engine=direct` that prints the set of `{ source, sha, skills, destPaths }` it _would_ install, without writing anything. Compare against today's `skills@latest add` output for a handful of real projects.
